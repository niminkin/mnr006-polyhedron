...
class Edge:
    ...
    def length(self):
        return (sqrt((self.fin.x - self.beg.x) ** 2 +
                     ...
                     (self.beg.z - self.beg.z) ** 2))

    # Центр ребра
    def edge_center(self):
        return R3((self.beg.x + self.fin.x) / 2,
                  (self.beg.y + self.fin.y) / 2,
                  (self.beg.z + self.fin.z) / 2)

    # Определение "хороших" рёбер
    def is_good_edge(self):
        if (R3.vertex_is_good(self.beg, 3) and
            R3.vertex_is_good(self.fin, 3)):
            if ((R3.vertex_is_good(self.beg, 1) or
                R3.vertex_is_good(self.fin, 1)) and
                R3.vertex_is_good(self.edge_center, 1)):
                return True
        return False
...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        # список "оригинальных" вершин полиэдра (без учета
        # поворота на углы Эйлера и коэффициента гомотетии)
    ...
    # Метод подсчета суммарной длины "хороших" рёбер
    def length_count(self):
        total_length = 0
        for i in self.edges:
            if Edge.is_good_edge(i):
                total_length += Edge.length(i)