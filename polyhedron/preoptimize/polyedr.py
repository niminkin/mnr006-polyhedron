...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        # списки "оригинальных" вершин и рёбер полиэдра
        self.original_vertexes, self.original_edges = [], []
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    original_vertexes = list(
                        self.original_vertexes[int(n) - 1] for n in buf)
                    # задание рёбер грани
                    for n in range(size):
                        self.edges.append(Edge(vertexes[n - 1], vertexes[n]))
                        self.original_edges.append(Edge(original_vertexes[n - 1],
                        original_vertexes[n]))
                    # задание самой грани
    ...
    # Метод подсчета суммарной длины
    # "хороших" рёбер
    def good_edges_length(self):
        total_length = 0
        counted_begs = [R3(10.0, 10.0, 10.0)]
        counted_fins = [R3(10.0, 10.0, 10.0)]
        counted_edges = 1
        for e in self.original_edges:
            beg = e.beg
            fin = e.fin
            if (beg.vertex_is_not_good(1) or fin.vertex_is_not_good(1) and\
                (beg.mid_dot(fin)).vertex_is_not_good(1)):
                edge_is_unique = True
                for i in range(0, counted_edges):
                    if ((counted_begs[i].is_equal(beg) and
                        counted_fins[i].is_equal(fin)) or
                        (counted_begs[i].is_equal(fin) and
                        counted_fins[i].is_equal(beg))):
                        edge_is_unique = False
                        break
                if (edge_is_unique):
                    total_length += beg.length(fin)
                    counted_begs.append(beg)
                    counted_fins.append(fin)
                    counted_edges += 1
        return (total_length)